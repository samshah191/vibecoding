...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { page = 1, limit = 10, search, isPublic, sortBy = 'updatedAt', sortOrder = 'desc' } = req.query;
...
router.get('/:id/versions', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        if (!userId) {
...
router.post('/:id/versions/:versionId/restore', auth_1.authenticateToken, async (req, res) => {
    try {
