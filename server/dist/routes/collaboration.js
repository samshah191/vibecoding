...
router.put('/:id/collaborators/:collaboratorId', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.delete('/:id/collaborators/:collaboratorId', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id, collaboratorId } = req.params;
        const userId = req.user?.userId;
        if (!userId) {
...
router.post('/:id/fork', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        if (!userId) {
