...
function isAdmin(userId) { return (process.env.ADMIN_USER_IDS || '').split(',').map(s => s.trim()).filter(Boolean).includes(userId); }
function startNewPeriod(userId, cycle) {
    const now = new Date();
...
function ensureUsage(userId, cycle) {
    const u = usageByUser.get(userId);
    if (!u)
        startNewPeriod(userId, cycle);
    else if (new Date(u.periodEnd) < new Date())
        startNewPeriod(userId, cycle);
    return usageByUser.get(userId);
}
function getPlan(planId) { const p = plans.find(p => p.id === planId); if (!p)
    throw new Error('Plan not found'); return p; }
function getQuota(userId) {
    const sub = subscriptionsByUser.get(userId);
    if (!sub)
        return plans[0].quotas; // default free
...
function prorate(oldCents, newCents, cycle, startedAt, renewalAt) {
    const now = new Date();
    const start = new Date(startedAt);
    const end = new Date(renewalAt);
...
// Subscription info
router.get('/subscription', auth_1.authenticateToken, (req, res) => {
    const sub = subscriptionsByUser.get(req.user.userId) || null;
    res.json({ success: true, data: sub });
});
router.post('/subscribe', auth_1.authenticateToken, (req, res) => {
    const schema = zod_1.z.object({ planId: zod_1.z.string(), cycle: zod_1.z.enum(['monthly', 'annual']).default('monthly') });
    const p = schema.parse(req.body);
    const plan = getPlan(p.planId);
    const now = new Date();
    const renewalAt = (p.cycle === 'monthly' ? addMonths(now, 1) : addYears(now, 1)).toISOString();
    const sub = { userId: req.user.userId, planId: plan.id, status: 'active', cycle: p.cycle, startedAt: nowIso(), renewalAt };
    subscriptionsByUser.set(req.user.userId, sub);
    startNewPeriod(req.user.userId, p.cycle);
    res.status(201).json({ success: true, data: sub });
});
router.post('/subscription/change', auth_1.authenticateToken, (req, res) => {
    const schema = zod_1.z.object({ planId: zod_1.z.string() });
    const p = schema.parse(req.body);
    const sub = subscriptionsByUser.get(req.user.userId);
    if (!sub)
        return res.status(400).json({ success: false, error: 'No active subscription' });
    const oldPlan = getPlan(sub.planId);
    const newPlan = getPlan(p.planId);
    ...
    sub.renewalAt = (sub.cycle === 'monthly' ? addMonths(new Date(), 1) : addYears(new Date(), 1)).toISOString();
    subscriptionsByUser.set(req.user.userId, sub);
    res.json({ success: true, data: { subscription: sub, proration: pr } });
});
router.post('/subscription/cancel', auth_1.authenticateToken, (req, res) => {
    const sub = subscriptionsByUser.get(req.user.userId);
    if (!sub)
        return res.status(400).json({ success: false, error: 'No active subscription' });
    sub.status = 'canceled';
...
router.post('/subscription/resume', auth_1.authenticateToken, (req, res) => {
    const sub = subscriptionsByUser.get(req.user.userId);
    if (!sub)
        return res.status(400).json({ success: false, error: 'No subscription' });
    sub.status = 'active';
    subscriptionsByUser.set(req.user.userId, sub);
    res.json({ success: true, data: sub });
});
// Usage & Credits
router.get('/usage', auth_1.authenticateToken, (req, res) => {
    const sub = subscriptionsByUser.get(req.user.userId);
    const cycle = sub?.cycle || 'monthly';
...
router.get('/check', auth_1.authenticateToken, (req, res) => {
    const schema = zod_1.z.object({ type: zod_1.z.enum(['ai', 'build', 'project', 'seat']), amount: zod_1.z.coerce.number().default(1) });
    const p = schema.parse(req.query);
    const rem = remaining(req.user.userId);
    const map = { ai: rem.aiTokens, build: rem.deployMinutes, project: rem.projects, seat: rem.collabSeats };
    const allowed = map[p.type] >= p.amount;
    res.json({ success: true, data: { allowed, remaining: map[p.type] } });
});
router.post('/usage/add', auth_1.authenticateToken, (req, res) => {
    const schema = zod_1.z.object({ type: zod_1.z.enum(['ai', 'build', 'project', 'seat']), amount: zod_1.z.number().positive() });
    const p = schema.parse(req.body);
    const sub = subscriptionsByUser.get(req.user.userId);
    const cycle = sub?.cycle || 'monthly';
    const u = ensureUsage(req.user.userId, cycle);
    const rem = remaining(req.user.userId);
    const mapRem = { ai: rem.aiTokens, build: rem.deployMinutes, project: rem.projects, seat: rem.collabSeats };
    if (mapRem[p.type] < p.amount)
        return res.status(429).json({ success: false, error: 'Quota exceeded', throttle: true, remaining: mapRem[p.type] });
    if (p.type === 'ai')
        u.aiTokens += p.amount;
    else if (p.type === 'build')
        u.deployMinutes += p.amount;
    else if (p.type === 'project')
        u.projects += p.amount;
    else if (p.type === 'seat')
        u.collabSeats += p.amount;
    u.updatedAt = nowIso();
    usageByUser.set(req.user.userId, u);
    res.json({ success: true, data: { usage: u, remaining: remaining(req.user.userId) } });
});
router.post('/usage/reset', auth_1.authenticateToken, (req, res) => {
    if (!isAdmin(req.user.userId))
        return res.status(403).json({ success: false, error: 'Admin only' });
    const schema = zod_1.z.object({ userId: zod_1.z.string(), cycle: zod_1.z.enum(['monthly', 'annual']).default('monthly') });
    const p = schema.parse(req.body);
    startNewPeriod(p.userId, p.cycle);
    res.json({ success: true, data: usageByUser.get(p.userId) });
});
// Invoices & overrides
router.get('/invoices', auth_1.authenticateToken, (req, res) => {
    const userId = req.user.userId;
    res.json({ success: true, data: invoices.filter(i => i.userId === userId) });
});
router.post('/invoices/generate', auth_1.authenticateToken, (req, res) => {
    const userId = req.user.userId;
    const sub = subscriptionsByUser.get(userId);
    if (!sub)
        return res.status(400).json({ success: false, error: 'No subscription' });
    ...
    if (overBuild > 0)
        items.push({ description: `Build minutes overage (${overBuild})`, amountCents: overBuild * plan.overage.deployPerMinuteCents });
    const subtotal = items.reduce((s, i) => s + i.amountCents, 0);
    const inv = { id: id(), userId, periodStart: u.periodStart, periodEnd: u.periodEnd, items, subtotalCents: subtotal, totalCents: subtotal, createdAt: nowIso() };
    invoices.push(inv);
    res.status(201).json({ success: true, data: inv });
});
router.get('/invoices/:id/export', auth_1.authenticateToken, (req, res) => {
...
router.post('/admin/override-subscription', auth_1.authenticateToken, (req, res) => {
    if (!isAdmin(req.user.userId))
        return res.status(403).json({ success: false, error: 'Admin only' });
    const schema = zod_1.z.object({ userId: zod_1.z.string(), planId: zod_1.z.string(), overrides: zod_1.z.object({ projects: zod_1.z.number().optional(), aiTokens: zod_1.z.number().optional(), deployMinutes: zod_1.z.number().optional(), collabSeats: zod_1.z.number().optional() }).optional() });
    const p = schema.parse(req.body);
    const existing = subscriptionsByUser.get(p.userId);
    if (!existing)
        return res.status(404).json({ success: false, error: 'Subscription not found' });
    existing.planId = p.planId;
    existing.overrides = p.overrides;
    subscriptionsByUser.set(p.userId, existing);
    res.json({ success: true, data: existing });
});
router.post('/admin/credit-grant', auth_1.authenticateToken, (req, res) => {
    if (!isAdmin(req.user.userId))
        return res.status(403).json({ success: false, error: 'Admin only' });
    const schema = zod_1.z.object({ userId: zod_1.z.string(), credits: zod_1.z.object({ aiTokens: zod_1.z.number().optional(), deployMinutes: zod_1.z.number().optional(), projects: zod_1.z.number().optional(), collabSeats: zod_1.z.number().optional() }) });
    const p = schema.parse(req.body);
    const sub = subscriptionsByUser.get(p.userId);
    const cycle = sub?.cycle || 'monthly';
    const u = ensureUsage(p.userId, cycle);
    if (p.credits.aiTokens)
        u.aiTokens = Math.max(0, u.aiTokens - p.credits.aiTokens);
    if (p.credits.deployMinutes)
        u.deployMinutes = Math.max(0, u.deployMinutes - p.credits.deployMinutes);
    if (p.credits.projects)
        u.projects = Math.max(0, u.projects - p.credits.projects);
    if (p.credits.collabSeats)
        u.collabSeats = Math.max(0, u.collabSeats - p.credits.collabSeats);
    u.updatedAt = nowIso();
    usageByUser.set(p.userId, u);
