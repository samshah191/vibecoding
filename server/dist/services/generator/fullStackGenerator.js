...
exports.FullStackGenerator = void 0;
// @ts-nocheck
...
async function writeFiles(baseDir, files) {
    for (const [relativePath, contents] of Object.entries(files)) {
        const absolute = path_1.default.join(baseDir, relativePath);
        await fs_extra_1.default.ensureDir(path_1.default.dirname(absolute));
        await fs_extra_1.default.writeFile(absolute, contents, 'utf8');
    }
}
class FullStackGenerator {
    constructor(baseDir = undefined) {
        const defaultBase = path_1.default.resolve(process.cwd(), '..', 'generated-projects');
        this.outputRoot = baseDir ? path_1.default.resolve(baseDir) : defaultBase;
    }
    async generateProject(options) {
        ...
        const projectRoot = path_1.default.join(this.outputRoot, appId);
        const clientPath = path_1.default.join(projectRoot, platform === 'mobile' ? 'mobile' : 'client');
        const serverPath = path_1.default.join(projectRoot, 'server');
        ...
        const archivePath = path_1.default.join(this.outputRoot, `${appId}.zip`);
...
Happy building!`;
    ...
    async createArchive(sourceDir, archivePath) {
        await fs_extra_1.default.ensureDir(path_1.default.dirname(archivePath));
    ...
    getArchivePath(appId) {
        return path_1.default.join(this.outputRoot, `${appId}.zip`);
    }
    getProjectRoot(appId) {
        return path_1.default.join(this.outputRoot, appId);
    }
    getOutputRoot() {
        return this.outputRoot;
    }
}
exports.FullStackGenerator = FullStackGenerator;
