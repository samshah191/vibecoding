import { CodeArtifactBundle, DatabaseArtifact, DocsArtifact, FileArtifact, GeneratedArtifacts, InfraArtifact } from '../../types/orchestration';

// Deterministic builders that do not call external LLMs. They transform inputs into stable outputs.
...
function file(path: string, content: string): FileArtifact { return { path, content }; }

export class ArtifactGenerators {
  static codeBundle(input: CodegenInput): CodeArtifactBundle {
    ...
    const files: FileArtifact[] = [
      ...
      file(`src/components/PricingPlans.${ext}`,
      ...
      file(`src/components/UserManagement.${ext}`,
      ...
      file(`src/components/AccessibilityScanner.${ext}`,
    ...
    const tests: FileArtifact[] = [
      file(`tests/smoke.test.${input.language === 'TypeScript' ? 'ts' : 'js'}`,
  ...
  static db(description: string): DatabaseArtifact {
    ...
    const migrations: FileArtifact[] = [file('migrations/0001_init.sql', '-- create tables; deterministic stub')];
